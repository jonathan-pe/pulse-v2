...
Triggered externally by `.github/workflows/ingest.yml` (cron every 15 min), one GitHub Actions matrix job per league hitting `POST /api/ingest/:league` in parallel, guarded by a shared-secret bearer token (`INGEST_SECRET`) rather than session auth. The unscoped `POST /api/ingest` (all leagues, sequential) exists only for manual/local testing — production always uses the per-league route.
